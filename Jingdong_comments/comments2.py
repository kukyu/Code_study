...
link = f'https://club.jd.com/comment/productPageComments.action?callback=fetchJSON_comment98&productId={productId}&score=0&sortType=5&page=0&pageSize=10&isShadowSku=0&fold=1'
...
maxPage = dictData["maxPage"]
# print(maxPage)
for page in range(1, maxPage+1):
    link = f'https://club.jd.com/comment/productPageComments.action?callback=fetchJSON_comment98&productId={productId}&score=0&sortType=5&page={page-1}&pageSize=10&isShadowSku=0&fold=1'
    requestData = requests.get(url=link, headers=uaAgent).text
    requestData = requestData.replace('fetchJSON_comment98(', "")
    requestData = requestData.replace(');', "")
    # print(requestData)
    ...
    myData.index += 1
    myData.to_csv("./Jingdong_comments/京东评论.csv", mode="a", header=0, encoding="ANSI")
    time.sleep(3)
